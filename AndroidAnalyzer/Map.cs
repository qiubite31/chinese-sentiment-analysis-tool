...
namespace AndroidAnalyzer
{
    public class Map
    {
        ...
        public Map(int dimensions, int length, string file,Form1 form)
        {
            this.length = length;
            ...
            LoadData(file);
            NormalisePatterns();
            Train(0.0000001,form);
            DumpCoordinates(form);
        }
        ...
        private void Train(double maxError, Form1 form)
        {
            ...
            while (currentError > maxError)
            {
                ...
                form.UpdateMSG(currentError.ToString("0.0000000"));
            }
        }
        ...
        private void DumpCoordinates(Form1 form)
        {
            for (int i = 0; i < patterns.Count; i++)
            {
                Neuron n = Winner(patterns[i]);
                //Console.WriteLine("{0},{1},{2}", labels[i], n.X, n.Y);
                form.UpdateMSG(labels[i].ToString() + "," + n.X.ToString() + "," + n.Y.ToString());
            }
        }
